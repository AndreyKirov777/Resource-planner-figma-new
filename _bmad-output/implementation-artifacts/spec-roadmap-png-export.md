...
Canvas redraw is required so chrome cannot leak. Reuse the same `rows` / `periodWidth` / `showLaneBars` / phases already computed in `Roadmap.tsx`. Phase band colors and accent `#8f4f8f` / amber `#d97706` / lane bar `#33627D` match the live timeline. Spread hatch via canvas pattern.

...
- **`daysInFTE` semantics.** The data/conversion docs treat `daysInFTE` as load-bearing (`weeksPerMonth = daysInFTE / 5`), but `issues-tasks.md` M1 reports it is never used in cost/effort calculations (hours hardcoded at 8/day, 40/week). Should `daysInFTE` drive the pricing/effort math, or is it cadence-only? CAP-4 and CAP-7 depend on the answer.

...
Feature ships and works (`docs/bmad-archive/specs/spec-roadmap/SPEC.md` CAP-13), but is frozen at
...
- source_spec: `docs/bmad-archive/implementation-artifacts/spec-planning-table-column-visibility.md`
...
- source_spec: `docs/bmad-archive/implementation-artifacts/spec-roadmap-vertical-drag.md`
...
- source_spec: `docs/bmad-archive/implementation-artifacts/spec-wbs-role-columns.md`
...
- source_spec: `docs/bmad-archive/implementation-artifacts/spec-roadmap-milestone-timeline-labels.md`
...
- source_spec: `docs/bmad-archive/implementation-artifacts/spec-wbs-reconciliation-compact-panel.md`
...
- source_spec: `docs/bmad-archive/implementation-artifacts/spec-fix-margin-on-currency-change.md`
...
- source_spec: `docs/bmad-archive/implementation-artifacts/spec-daily-exchange-rates.md`
...
- source_spec: `docs/bmad-archive/implementation-artifacts/spec-roadmap-bar-name-labels.md`
...
- source_spec: `docs/bmad-archive/implementation-artifacts/spec-roadmap-unlinked-outline-switch.md`
...
## Carried from the TEA release gate (gate FAIL, evaluated 2026-08-27)

`docs/bmad-archive/test-artifacts/gate-decision.json`: P0 coverage is 100%, but overall
FULL coverage is 74% (minimum 80%) and P1 FULL coverage is 71% (minimum 80%). This is a
test-evidence gap, not a finding that anything is unimplemented — the TEA skills that
produced this gate are removed in phase 4 of the OpenSpec migration; these concrete gaps
are the part worth keeping.

- **Weekly/monthly conversion has no API or `modeConversion.ts` unit tests.**
- **Project copy is untested.**
- **`generate-plan` HTTP endpoint and its UI are untested** beyond the mock-model unit tests in `spec-1b`/`spec-2`.
- **Client-view chrome is only partly proven.**
- **The roadmap item editor is only partly proven.**

## Carried from the umbrella SPEC's Open Questions (`spec-resource-planner`, as of 2026-09-10)

- **Authentication/authorization scope.** `issues-tasks.md` H6 flags the API as fully open.
  Is auth/authz in near-term scope? It would reshape every endpoint and the data model, so
  it needs a human decision before downstream work assumes either answer.
- **AI generation guardrails.** Confirm the v1 default provider/model (Anthropic
  `claude-opus-4-8`) and the concrete cost/rate-limit guardrail values (per-user throttle;
  `429` on exceed) for `ai.config.ts`.

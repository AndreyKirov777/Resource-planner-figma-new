...
- **HTTPS:** TEST uses Caddy (`COMPOSE_PROFILES=tls`) at `https://resource-planner-test.dataart.com` with the IPA-issued PEM in `certs/`. The legacy IPA hostname still uses `tls internal`.

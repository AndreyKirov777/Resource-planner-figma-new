...
- [x] Вынести фабрику `makeFieldColumn(field, header, transform?)` для 6-кратного дублирования `onCellValueChanged` в ResourceList — `src/components/ResourceList.tsx:78-185`. Сигнатура расширена до `makeFieldColumn(field, header, { transform?, ...остальные ColDef-поля })` — одного `transform` не хватало для Location (`cellEditor`/`cellEditorParams`/`valueFormatter`/`tooltipValueGetter`) и Hourly cost (`valueFormatter`), нужно было передавать и их. `Daily cost` (вычисляемая, без поля/onCellValueChanged) в фабрику не заворачивал — осталась как есть.
- [x] Перенести `getAllocationBgColor` в `gridTheme.ts` (уже существующее общее место для темы грида), убрать копию — `src/components/ClientView.tsx:17-29` vs `src/components/ResourcePlan.tsx:80-92`. Обе копии были побайтово идентичны — перенос без изменений в логике.
